...
function addBrightnessContrastAdjustment(brightVal, contrVal, layerName, targetGroup, clipToLayer) {
...
function addVibranceAdjustment(vibVal, satVal, layerName, targetGroup, clipToLayer) {
...
function addLevelsAdjustmentLayer(blackPoint, whitePoint, gamma, targetGroup) {
  ...
  lvlDesc.putEnumerated(stringIDToTypeID('presetKind'), stringIDToTypeID('presetKindType'), stringIDToTypeID('presetKindDefault'));
  ...
  valsDesc.putEnumerated(stringIDToTypeID('presetKind'), stringIDToTypeID('presetKindType'), stringIDToTypeID('presetKindCustom'));
...
function placeImageInDocument(
  ...
  if (reuseCache && reuseCache[imagePath]) {
    ...
    if (typeof posX !== 'undefined' && typeof posY !== 'undefined') {
      cachedLayer.translate(posX - cachedLayer.bounds[0].as('px'), posY - cachedLayer.bounds[1].as('px'));
    }
  ...
  if (targetWidthPx && targetHeightPx) {
    if (targetWidthPx > 36000 || targetHeightPx > 36000) {
      alert('⚠️ Skipping image due to unreasonable size: ' + imageFile.name);
      imgDoc.close(SaveOptions.DONOTSAVECHANGES);
      return null;
    }

    imgDoc.resizeImage(UnitValue(targetWidthPx, 'px'), UnitValue(targetHeightPx, 'px'), null, ResampleMethod.BICUBIC);
  }
  // Crop margins from all sides if requested (e.g., to remove bleed)
  if (cropMarginPx && cropMarginPx > 0) {
    var cropWidth = imgDoc.width.as('px') - cropMarginPx * 2;
    var cropHeight = imgDoc.height.as('px') - cropMarginPx * 2;

    if (cropWidth <= 0 || cropHeight <= 0) {
      logError('Skipped cropping due to invalid crop margin on image: ' + imageFile.name);
      imgDoc.close(SaveOptions.DONOTSAVECHANGES);
      return null;
    }

    imgDoc.crop([cropMarginPx, cropMarginPx, imgDoc.width - cropMarginPx, imgDoc.height - cropMarginPx]);

    if (imgDoc.width.as('px') > 36000 || imgDoc.height.as('px') > 36000) {
      logError('Error - Image too large after crop: ' + imageFile.name);
      alert('⚠️ Image too large after crop: ' + imageFile.name);
      imgDoc.close(SaveOptions.DONOTSAVECHANGES);
      return null;
    }
  }
  ...
  if (typeof posX !== 'undefined' && typeof posY !== 'undefined') {
    newLayer.translate(posX - newLayer.bounds[0].as('px'), posY - newLayer.bounds[1].as('px'));
  }
...
function drawCropMark(x, y, size, armThickness, armLength) {
  ...
  var thickness = typeof armThickness === 'number' ? armThickness : Math.round(size / 11);
...
function drawCardBackground(x, y, width, height, fillColor, layerName, targetGroup) {
  var doc = app.activeDocument;
...
function shiftEntireDocumentByOffset(xOffsetPx, yOffsetPx) {
  ...
  for (var i = 0; i < doc.artLayers.length; i++) {
    ...
    if (!layer.isBackgroundLayer && name.indexOf('brightness/contrast for card') !== 0 && name.indexOf('vibrance for card') !== 0) {
      try {
        layer.translate(xOffsetPx, yOffsetPx);
      } catch (e) {
        // Skip on error
        logError('Error - shiftEntireDocumentByOffset - Move top-level artlayers');
  ...
  for (var j = 0; j < doc.layerSets.length; j++) {
    ...
    if (groupName !== 'sheet adjustments') {
      try {
        ...
        // Skip on error
        logError('Error - shiftEntireDocumentByOffset - Move top-level groups');
        logError('e: ' + e.message);
      }
    }
  }
...
function placeSilhouettePSDLayer(doc, scriptFolder) {
  try {
    ...
    logError('Error - placeSilhouettePSDLayer');
    logError('e: ' + e.message);
...
function placeSilhouetteDetectionAid(doc, scriptFolder, x, y, dpi) {
  try {
    ...
    imgDoc.resizeImage(UnitValue(targetWidth, 'px'), UnitValue(targetHeight, 'px'), null, ResampleMethod.BICUBIC);
    ...
    pastedLayer.translate(x - pastedLayer.bounds[0].as('px'), y - pastedLayer.bounds[1].as('px') - layerHeight / 2.25);
    pastedLayer.move(doc, ElementPlacement.PLACEATBEGINNING);
  } catch (e) {
    logError('Error - placeSilhouetteDetectionAid');
    logError('e: ' + e.message);
...
function addNoBleedPadding(doc, bleedSizePx, mode) {
  try {
    ...
    logError('Error - addNoBleedPadding');
    logError('e: ' + e.message);
    // Fails silently
  }
}
...
function getEdgeAverageDarkColor(doc) {
  ...
  try {
    ...
    logError('Error - getEdgeAverageDarkColor');
    logError('e: ' + e.message);
    // Fails silently
...
function exportSinglesFromFolder(config) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (cropMarginPx > 0) {
        doc.crop([cropMarginPx, cropMarginPx, doc.width - cropMarginPx, doc.height - cropMarginPx]);
      }
      ...
      try {
        ...
        logError('Error - gexportSinglesFromFolder - Find Bleed Fill layer and move it below the image as final step');
        logError('e: ' + e.message);
      }
...
function saveBatchHistory(configVars, scriptFolder, cardFilesToSave, batchNumberToUse) {
  try {
    ...
    logError('Error - saveBatchHistory');
    logError('e: ' + e.message);
...
function logError(message) {
  try {
    ...
    if (!opened) {
      alert('⚠️ Could not open log file: ' + logFile.fsName);
      return;
    }
